...
/// waiting queue. To acquire a MCS-CR lock, an instance of queue node must be
...
///
/// use malthlock::raw::{self, MutexNode};
...
impl<T: ?Sized, R: Relax> Mutex<T, R> {
    ...
    /// unlocked once the closure goes out of scope.
    ///
    /// To acquire a MCS-CR lock through this function, it's also required a mutably
